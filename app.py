...
st.set_page_config(page_title="Uzun Kod — v18 / Statik", page_icon="🧩", layout="wide", initial_sidebar_state="collapsed")
...
with left:
    st.title("Uzun Kod Oluşturma Programı - v18 / Statik")
...
if st.session_state["step"]==1:
    ...
    if prods.empty:
        ...
        if isinstance(prev, dict):
            prev_disp = prev["UrunAdi"]+" — "+prev["MakineTipi"]
            if prev_disp in display: idx = display.index(prev_disp)
        sel = st.radio("Ürün", options=display, index=idx if idx is not None else 0, key="product_radio")
        if sel:
                ...
                with tab:
                    ...
                    with cols[0]:
                        ...
                        if fields.empty:
                            ...
                            if (block_idx is not None) and (i>block_idx):
                                st.warning(f"🔒 Önce **{statuses[block_idx][1]}** bölümündeki zorunlu alanları tamamlayın.")
                            else:
                                for _,fld in fields.iterrows():
                    ...
                    with cols[1]:
                        ...
                        if fname and os.path.exists(pth):
                            st.image(pth, caption=sec_label, use_container_width=True)
                        else:
